...
- **Must-do:** sections 1–7 (~42–50 hrs). That's a complete, differentiated
  artifact on its own.
- **Cut:** the LLM hypothesis layer — reads as AI-washing to chemistry PhDs.
- **Protect** the ceiling-detection work (§5) and the write-up (§7) — they're
  what make this "saw something we didn't" rather than "reproduced a paper."
- **Report variance honestly** — technical founders scrutinize baselines and
  seeds hardest.
- **Priority order under time pressure:** the moment you lose a half-day, the
  UI goes first, then transfer learning — never §5.
